...
var logger = shim.NewLogger("HackathonChaincode")
...
//==============================================================================================================================
//	Item - Defines the structure for an item object. JSON on right tells it what JSON fields to map to
...
//==============================================================================================================================
//	Items Holder - Defines the structure that holds all the items that have been created.
...
//					JSON into the Item struct for use in the contract. Returns the Item struct.
//					Returns empty v if it errors.
//==============================================================================================================================
func (t *SimpleChaincode) retrieve_item(stub shim.ChaincodeStubInterface, sn string) (Item, error) {
    ...
    if err != nil {	fmt.Printf("RETRIEVE_ITEM: Corrupt item record "+string(bytes)+": %s", err); return item, errors.New("RETRIEVE_ITEM: Corrupt item record"+string(bytes))	}

	return item, nil
}

//==============================================================================================================================
// save_changes - Writes to the ledger the Item struct passed in a JSON format. Uses the shim file's
//				  method 'PutState'.
//==============================================================================================================================
func (t *SimpleChaincode) save_changes(stub shim.ChaincodeStubInterface, item Item) (bool, error) {
...
//        if function == "update_owner" {return t.update_owner(stub, item, caller, caller_affiliation, args[0])}
...
//}
	...
	sn         	   := "\"sn\":\""+args[0]+"\", "							// Variables to define the JSON
	manufacturer   := "\"manufacturer\""+args[1]+"\", "
	owner          := "\"owner\":\""+args[2]+"\", "
...
func (t *SimpleChaincode) get_item_details(stub shim.ChaincodeStubInterface, item Item, caller string, caller_affiliation string) ([]byte, error) {
	...
	if err != nil { return nil, errors.New("GET_ITEM: Invalid item object") }
...
//func (t *SimpleChaincode) get_items(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string) ([]byte, error) {
//	bytes, err := stub.GetState("allItems")
//
//	if err != nil { return nil, errors.New("Unable to get allItems") }
//
//	var allItems Items_Holder
//
//	err = json.Unmarshal(bytes, &allitems)
//
//	if err != nil {	return nil, errors.New("Corrupt Items_Holder") }
//
//	result := "["
//
//	var temp []byte
//	var item Item
//
//	for _, id := range allItems.items {
//
//		item, err = t.retrieve_item(stub, id)
//
//		if err != nil {return nil, errors.New("Failed to retrieve Item")}
//
//		temp, err = t.get_item_details(stub, item, caller, caller_affiliation)
//
//		if err == nil {
//			result += string(temp) + ","
//		}
